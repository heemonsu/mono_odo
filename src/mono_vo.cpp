...
void mono_vo::Detect_Match_Features(Mat img_1, Mat img_2, vector<Point2f>& match_points1, vector<Point2f>& match_points2, string type)  {
 ...
 if(type == "SIFT"){
  ...
  // C++: ORB::ORB(int nfeatures=500, float scaleFactor=1.2f, int nlevels=8, int edgeThreshold=31, int firstLevel=0, int WTA_K=2, int scoreType=ORB::HARRIS_SCORE, int patchSize=31)
  static Ptr<FeatureDetector> orb = ORB::create(); 
  orb->detectAndCompute(img_1,cv::noArray(), keypoints_1, descriptors_1); 
...
vector<Point3f> mono_vo::get3D_Points_0(const vector<Point2f>& feature_p1, const vector<Point2f>& feature_p2) {
...
vector<Point3f> mono_vo::get3D_Points_1(const vector<Point2f>& feature_p1, const vector<Point2f>& feature_p2, vector<Point2f>& prevFeatures, Mat inv_transform) {

  vector<Point3f> landmarks_new = get3D_Points_0(feature_p1, feature_p2);
  vector<Point3f> landmarks;
   	...
   	for (int k = 0; k < landmarks_new.size(); k++) {
	   	...
	   	if (p.z > 0) {
			...
			prevFeatures.push_back(feature_p1[k]);
	   	}
...
void mono_vo::Continious(const string& filename1, const string& filename2, const vector<vector<float>>& poses){
  ...
  Detect_Match_Features(img_1, img_2, match_points1, match_points2, type);
  landmarks = get3D_Points_0(match_points1, match_points2);
  ...
  vector<Point2f> prevFeatures = match_points1;
  ...
  for(int numFrame=4; numFrame < 1500; numFrame++)	{ 

	cout << "numFrame = " << numFrame << endl;

	sprintf(filename_1, "/media/mahdi/Bulk1/Ubuntu/dataset-kitti-odom/sequences/00/image_0/%06d.png", numFrame);
	sprintf(filename_2, "/media/mahdi/Bulk1/Ubuntu/dataset-kitti-odom/sequences/00/image_1/%06d.png", numFrame);

  	Mat currImage_c = imread(filename_1);
  	cvtColor(currImage_c, currImage, COLOR_BGR2GRAY);

  	Mat currImage_c_1 = imread(filename_2);
  	cvtColor(currImage_c_1, currImage_1, COLOR_BGR2GRAY);
	...
	vector<int> inliers;
  	...
  	solvePnPRansac(landmarks_ref, featurePoints_ref, K, dist_coeffs, rv, tv, false, 100, 8.0, 0.99, inliers);
	...
	Detect_Match_Features(currImage, currImage_1, mpoints1, mpoints2, type);
	...
	landmarks = get3D_Points_1(mpoints1, mpoints2, prevFeatures, inv_transform);

	cout << "landmarks AFTER CLEANING = " << landmarks.size() << endl;

	
	//prevFeatures = match_points1;
	prevImage = currImage;
